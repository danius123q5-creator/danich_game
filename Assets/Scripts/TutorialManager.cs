...
public class TutorialManager : MonoBehaviour
{
    ...
    static readonly string[] Hints =
    {
        ...
        Lang.T("Зажми Q — меню построек разбито на категории (СТРОЙКА, ДОБЫЧА, ЛОГИСТИКА, ТУРЕЛИ, ПВО, ЛОВУШКИ, ТЯЖЁЛОЕ). Выбери РАЗДАТЧИК (подсвечен) и поставь его ЛКМ. Это сердце базы — если его уничтожат, игра окончена. Защищай его!", "Hold Q — the build menu is split into categories (CONSTRUCTION, EXTRACTION, LOGISTICS, TURRETS, AIR DEFENSE, TRAPS, HEAVY). Pick the DISPENSER (highlighted) and place it with LMB. It's the heart of your base — if it's destroyed, the game is over. Defend it!"),
        ...
        Lang.T("Захвачено! НПЗ даёт НЕФТЬ — на ней работают супер-пушки (Тесла, Орбиталка). ШАХТЫ дают металл. Точки захватывай и ОБОРОНЯЙ — или построй свои НЕФТ. ВЫШКУ / БУРОВУЮ (захват не нужен). Труба/конвейер тянутся линией (зажми ЛКМ у источника, веди к базе) — идут по осям и делают УГОЛ (не по диагонали), R выбирает сторону поворота. Дозатор/чан у базы сами выдают ресурс. Чем больше источников на сети — тем больше потока.", "Captured! A refinery yields OIL — it powers super-weapons (Tesla, Orbital Strike). MINES yield metal. Capture points and DEFEND them — or build your own OIL DERRICK / DRILL RIG (no capture needed). Pipes/conveyors run in a line (hold LMB at the source, drag to the base) — they follow the axes and turn a CORNER (not diagonally), R picks which side the corner is on. The dispenser/vat at the base output the resource. The more sources on the network — the more flow."),
        Lang.T("Построй СТЕНУ (подсвечена) — она задержит зомби. Совет: зажми ЛКМ и проведи линию — стена встанет целым РЯДОМ за раз. Клавиша R поворачивает постройку на 90°.", "Build a WALL (highlighted) — it holds the zombies back. Tip: hold LMB and drag a line to lay a whole ROW at once. Press R to rotate a piece 90°."),
        Lang.T("Построй ТУРЕЛЬ (подсвечена) — она стреляет по зомби сама. Турели, ловушки и стены тоже можно тянуть ЛКМ рядами и ставить прямо НА верх стен.", "Build a TURRET (highlighted) — it fires at zombies on its own. Turrets, traps and walls can also be dragged in rows with LMB, and placed right ON TOP of walls."),
